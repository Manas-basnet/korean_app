...
class ChapterAudioTracksWidget extends StatefulWidget {
  final String bookId;
  final Chapter chapter;
  final bool isCompact;
  final bool showPreloadButton;

  const ChapterAudioTracksWidget({
    super.key,
  ...
  @override
  State<ChapterAudioTracksWidget> createState() => _ChapterAudioTracksWidgetState();
}

class _ChapterAudioTracksWidgetState extends State<ChapterAudioTracksWidget> {
  ...
  Widget build(BuildContext context) {
    if (widget.chapter.audioTracks.isEmpty) {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
                ...
                Expanded(
                  child: Text(
                    _languageCubit.getLocalizedText(
                      korean: '챕터 오디오 (${widget.chapter.audioTracks.length})',
                      english: 'Chapter Audio (${widget.chapter.audioTracks.length})',
                    ),
            ...
            ...widget.chapter.audioTracks.map((audioTrack) => 
  ...
  void _handleAudioTrackLoadingState(KoreanBooksState state) {
    ...
    if (operation.type == KoreanBooksOperationType.loadAudioTrack) {
      final trackKey = operation.bookId;

      if (operation.status == KoreanBooksOperationStatus.inProgress) {
        if (mounted) {
          setState(() {
            _loadingTracks.add(state.loadedAudioTrackId ?? '');
          });
        }
      } else if (operation.status == KoreanBooksOperationStatus.completed &&
                 state.loadedAudioFile != null &&
                 state.loadedAudioBookId == widget.bookId &&
                 state.loadedAudioChapterId == widget.chapter.id) {
        if (mounted) {
          setState(() {
            if (state.loadedAudioTrackId != null) {
              _loadingTracks.remove(state.loadedAudioTrackId!);
        ...
        if (mounted && state.loadedAudioTrackId != null) {
          setState(() {
            _loadingTracks.remove(state.loadedAudioTrackId!);
          });
        }
    ...
    if (operation.type == KoreanBooksOperationType.preloadAudioTracks &&
        operation.bookId == widget.chapter.id) {
      if (operation.status == KoreanBooksOperationStatus.completed) {
        _snackBarCubit.showSuccessLocalized(
          korean: '모든 챕터 오디오 트랙이 다운로드되었습니다',
          english: 'All chapter audio tracks downloaded successfully',
        );
      } else if (operation.status == KoreanBooksOperationStatus.failed) {
        _snackBarCubit.showErrorLocalized(
          korean: '챕터 오디오 트랙 다운로드에 실패했습니다',
          english: 'Failed to download chapter audio tracks',
        );
      }
    }
  }
  ...
  void _loadAudioTrack(String audioTrackId) {
    _koreanBooksCubit.loadChapterAudioTrack(widget.bookId, widget.chapter.id, audioTrackId);
  }

  void _preloadAllTracks() {
    _koreanBooksCubit.preloadChapterAudioTracks(widget.bookId, widget.chapter.id);
  }
}